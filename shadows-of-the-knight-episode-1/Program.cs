...
class Player
{
    static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            {
                x = currX;
                y = maxJumpHigh == -1 ? (currY + 1) / 2 : currY - maxJumpHigh;
                maxJumpHigh = maxJumpHigh == -1 ? ((currY + 1) / 2) :maxJumpHigh;
            ...
            {
                x = currX;
                y = maxJumpHigh == -1 ? (currY + ((H - currY + 1) / 2)) : currY + maxJumpHigh;
                maxJumpHigh = maxJumpHigh == -1 ? ((H - currY + 1) / 2) : maxJumpHigh;
                maxJumpHigh = maxJumpHigh / 2;
                maxJumpHigh = maxJumpHigh <= 0 ? 1 : maxJumpHigh;
            }
            if (y < 0)
            ...
            {
                x = maxJumpWidth == -1 ? (currX + 1) / 2 : currX - maxJumpWidth;
                y = currY;
                maxJumpWidth = maxJumpWidth == -1 ? (currX + 1) / 2 : maxJumpWidth;
                maxJumpWidth = maxJumpWidth / 2;
                maxJumpWidth = maxJumpWidth == 0 ? 1 : maxJumpWidth;
            }
            else if (bombDir.IndexOf("R") > -1)
            {
                x = maxJumpWidth == -1 ? (currX + ((W - currX + 1) / 2)) : currX + maxJumpWidth;
                y = currY;
                maxJumpWidth = maxJumpWidth == -1 ? ((W - currX + 1) / 2) : maxJumpWidth;
                maxJumpWidth = maxJumpWidth / 2;
                maxJumpWidth = maxJumpWidth == 0 ? 1 : maxJumpWidth;
            }
            if (x < 0)
